...
static void read_disk(int sector, int sector_count) {
	outb(0x1F2, (uint8_t)(sector_count >> 8));	// 扇区数量
	outb(0x1F2, (uint8_t)(sector_count));		// 扇区数量，可以填0的
	...
	outb(0x1F6, (uint8_t)(0xE0));				// 驱动器0，LBA模式
	outb(0x1F7, (uint8_t)0x24);					// 读命令
	...
	while (sector_count-- > 0) {
		...
		// 读取并将数据写入到缓存中
		for (int i = 0; i < DISK_SIZE_PER_SECTOR / 2; i++) {
			*data_buf++ = inw(0x1F0);
		}
...
void boot_entry(void) {
	...
	// 加载根目录区，1个够了，足够包含loader的信息
	read_disk(start_secotr, 1);

	// 解析diritem，找到loader文件
	diritem_t *loader_item = (diritem_t*) LOADER_START_ADDR;
	diritem_t * end_item = loader_item + DISK_SIZE_PER_SECTOR / sizeof(diritem_t);
	do {
		// 找到根目录的第一个以L开头的文件，肯定是loader了
		// 因为要求安装系统时，loader作为第一个文件最先写入
		// 这里只检测了第个字符，有兴趣可以检测更多字符
		if (loader_item->DIR_Name[0] == 'L') {
			...
			start_secotr = start_secotr + fat_dbr.BPB_RootEntCnt * sizeof(diritem_t) / DISK_SIZE_PER_SECTOR
					+ (cluster - FAT_START_CLUSTER) * fat_dbr.BPB_SecPerClus;

			// 文件大小正好是512整数倍时，会多读 一扇区，不影响功能，只能多不能少
			// 但比(size + 511) / 512 更节省代码空间
			// read_disk(start_secotr, loader_item->DIR_FileSize / DISK_SIZE_PER_SECTOR + 1);
			read_disk(start_secotr, 200);    // 100KB 足够大了

			// 跳转至loader运行，相比函数调用，更加节省空间
			__asm__ __volatile__("jmp $0, %0"::"i"(LOADER_START_ADDR));
			//((void (*)()(LOADER_START_ADDR))();
		}
	} while (++loader_item < end_item);
