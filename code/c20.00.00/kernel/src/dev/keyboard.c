...
static void update_led_status (void) {
	...
	data |= key_mode.num_lock << 1;
	data |= key_mode.caps_lock << 0;
 ...
 * esc是以esc开头的的键值
 */
static const map_table_t map_table[] = {
...
static inline uint8_t r_keycode(uint8_t key_code) {
...
static void write_key (void) {
	// 获得的按键，发给UI去处理
	// 然后由UI转交给相应的控制终端
	ui_msg_t *msg = ui_alloc_msg(UI_MSG_KEYBOARD, 0);
	if (msg) {
		k_memcpy(&msg->code, &key_data, sizeof(key_data));
		ui_send_msg(msg, (task_t *)0, 0);

		// 其它位shift等不清除，因为键可能未释放
		key_data.key_code = 0;
		key_data.func = 0;
	}
}
...
static void do_e0_key (uint8_t raw_code) {
    int key = r_keycode(raw_code);			// 去掉最高位
...
static void do_normal_key (uint8_t raw_code) {
	...
	case KEY_NUMLOCK:  // 小键盘
		if (is_make) {
			key_mode.num_lock = ~key_mode.num_lock;
			update_led_status();
		}
		break;
	case KEY_CAPS:  // 大小写键，设置大小写状态
		if (is_make) {
			key_mode.caps_lock = ~key_mode.caps_lock;
			update_led_status();
		}
		break;
