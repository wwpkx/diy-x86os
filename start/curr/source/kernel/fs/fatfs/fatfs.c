...
int fatfs_mount(struct _fs_t * fs, int major, int minor) {
    ...
    if (kernel_memcmp(dbr->BS_FilSysType, "FAT16", 5) != 0) {
...
int fatfs_read (char * buf, int size, file_t * file) {
    ...
    while (nbytes > 0) {
        ...
        uint32_t start_secotr = fat->data_start + (file->cblk - 2) * fat->sec_per_cluster;

        if ((cluster_offset == 0) && (nbytes == fat->cluster_byte_size)) {
            int err = dev_read(fat->fs->dev_id, start_secotr, buf, fat->sec_per_cluster);
            if (err < 0) {
                return total_read;
            }
            ...
            fat->curr_sector = -1;
            int err = dev_read(fat->fs->dev_id, start_secotr, fat->fat_buffer, fat->sec_per_cluster);
            if (err < 0) {
                return total_read;
            }
...
int fatfs_write (char * buf, int size, file_t * file) {
    ...
    while (nbytes) {
        ...
        if ((cluster_offset == 0) && (nbytes == fat->cluster_byte_size)) {
            int err = dev_write(fat->fs->dev_id, start_secotr, buf, fat->sec_per_cluster);
            if (err < 0) {
                return total_write;
            }
            ...
            err = dev_write(fat->fs->dev_id, start_secotr, fat->fat_buffer, fat->sec_per_cluster);
            if (err < 0) {
                return total_write;
            }          
        }
