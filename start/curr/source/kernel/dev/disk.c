...
static void disk_send_cmd (disk_t * disk, uint32_t start_secotr, uint32_t sector_count, int cmd) {
    outb(DISK_DRIVE(disk), DISK_DRIVE_BASE | disk->drive);
    outb(DISK_SECTOR_COUNT(disk), (uint8_t)(sector_count >> 8));
    outb(DISK_LBA_LO(disk), (uint8_t)(start_secotr >> 24));
    outb(DISK_LBA_MID(disk), 0);
    outb(DISK_LBA_HI(disk), 0);

    outb(DISK_SECTOR_COUNT(disk), (uint8_t)(sector_count));
    outb(DISK_LBA_LO(disk), (uint8_t)(start_secotr));
    outb(DISK_LBA_MID(disk), (uint8_t)(start_secotr >> 8));
    outb(DISK_LBA_HI(disk), (uint8_t)(start_secotr >> 16));
